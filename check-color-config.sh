...
# Checking all colors with format #123abc, #1a9 are in the config colors.json (except for #fff).
readonly COLOR_REGEX="#[0-9a-f]{6}|#[0-9a-f]{3}(?<!fff)"
...
if [[ "$EXTRA_COLORS_COUNT" != "0" ]]; then
  ...
  grep -iP $COLOR_REGEX -rn --include \*.js --include \*.jsx --include \*.ts --include \*.tsx "$FRONTEND_FOLDER/src" | grep -v ^import
  exit 1
fi
